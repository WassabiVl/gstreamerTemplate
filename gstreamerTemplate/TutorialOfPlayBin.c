...
int main(int argc, char *argv[]) {
	 ...
	 * playbin is in itself a pipeline
	 **/
	data.playbin = gst_element_factory_make ("playbin", "playbin");

	/**
	 * 3) check if the playbin has been created
	 */
	if (!data.playbin) {
		g_printerr ("playbin elements could not be created.\n");
		return -1;
	}

	/**
	 * 4)Set the URI to play
	 * if its from a uri
	 **/
	g_object_set (data.playbin, "uri", "https://www.freedesktop.org/software/gstreamer-sdk/data/media/sintel_trailer-480p.webm", NULL);

	/**
	 * 5) Create the empty pipeline, in this example it is not needed
	 **/
//	data.pipeline = gst_pipeline_new ("test-pipeline");

	/**
	 * 6) check if the pipeline or elements have problems
...
//	g_signal_connect (data.source, "pad-added", G_CALLBACK (pad_added_handler), &data);
	...
	/**
	 * 9) Start playing
	 * this is an example for the play bin
	 **/
//	ret = gst_element_set_state (data.pipeline, GST_STATE_PLAYING);
	...
	do {
		...
		if (msg != NULL) {
			 ...
			 * We don't want to do anything if we are not in PLAYING state, because most queries would fail.
			 **/
			if (data.playing) {
				gint64 current = -1;
